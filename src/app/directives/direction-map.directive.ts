...
// other interface can be used for origin and destination, but it must contain latlng data
...
export class DirectionsMapDirective implements OnInit, OnChanges {
  ...
  //  keep a single google maps directions renderer instance so , it can reuse
  // using a new renderer instance every time will leave the previous one still active and visible on the page
  private directionsRenderer: any;

  // inject AGM's google maps api wrapper that handles the communication with the Google Maps Javascript
  ...
  drawDirectionsRoute() {
      this.gmapsApi.getNativeMap().then(map => {
        if (!this.directionsRenderer) {
          // if already have a marker at the coordinate location on the map, use suppressMarkers option
